<template>
	<div class="main-content" :style='{"minHeight":"100vh","padding":"0px 0px 20px","overflow":"hidden","color":"#666","background":"#f4f8fb","fontSize":"14px","position":"relative"}'>
				...
				@selection-change="selectionChangeHandler">
					...
					<el-table-column width="300" label="操作">
						<template slot-scope="scope">
							...
							<el-button class="btn8" v-if="isAuth('yonghu','营养评估')" @click="yingyangpingguCrossAddOrUpdateHandler(scope.row,'cross','','','','')" type="success">
		...
		<yingyangpinggu-cross-add-or-update v-if="yingyangpingguCrossAddOrUpdateFlag" :parent="this" ref="yingyangpingguCrossaddOrUpdate"></yingyangpinggu-cross-add-or-update>
...
import yingyangpingguCrossAddOrUpdate from "../yingyangpinggu/add-or-update";
	export default {
		data() {
			return {
				searchForm: {
				...
				yingyangpingguCrossAddOrUpdateFlag: false,
		...
		components: {
			AddOrUpdate,
			yingyangpingguCrossAddOrUpdate,
		},
		methods: {
			imgPreView(url){
				this.previewImg = url
			...
			yingyangpingguCrossAddOrUpdateHandler(row,type,crossOptAudit,crossOptPay,statusColumnName,tips,statusColumnValue){
				this.showFlag = false;
				this.addOrUpdateFlag = false;
				this.yingyangpingguCrossAddOrUpdateFlag = true;
				...
				if(statusColumnName!=''&&!statusColumnName.startsWith("[")) {
					...
					for (var o in obj){
						if(o==statusColumnName && obj[o]==statusColumnValue){
							...
							this.showFlag = true;
							this.yingyangpingguCrossAddOrUpdateFlag = false;
							return;
						}
					}
				}
				this.$nextTick(() => {
					this.$refs.yingyangpingguCrossaddOrUpdate.init(row.id,type);
				});
			},
